...
class TestStartCLI(unittest.TestCase):
    ...
    def test_start_cli_basic_commands(self, mock_handlers, mock_printc, mock_prompt_session):
        # Arrange
        mock_handler_func = MagicMock()
        mock_handlers.update({
            "help": mock_handler_func,
            "exit": mock_handler_func
        })
        ...
        mock_printc.assert_any_call("Welcome to Peek-A-Boo CLI. Type 'help' to see available commands.", level="headline")
